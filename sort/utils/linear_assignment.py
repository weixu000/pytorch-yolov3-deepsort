...
def min_cost_matching(
    ...
    matched_trackers, matched_detections = linear_sum_assignment(cost_matrix)

    unmatched_detections = [idx
                            for col, idx in enumerate(detection_indices)
                            if col not in matched_detections]
    unmatched_tracks = [idx
                        for row, idx in enumerate(track_indices)
                        if row not in matched_trackers]
    matches = []
    for row, col in zip(matched_trackers, matched_detections):
